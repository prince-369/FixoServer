...
exports.cldPreset = exports.cldUrl = void 0;
...
const isCloudinaryDeliveryUrl = (url) => {
...
const cldUrl = (url, transform = {}) => {
    ...
    if (TRANSFORM_TOKEN.test(firstSegment))
        return url;
    ...
    if (!transformStr)
        return url; // nothing meaningful to add
    return `${head}${transformStr}/${tail}`;
};
exports.cldUrl = cldUrl;
// Convenience presets for common render points (used during Phase 7 rollout).
exports.cldPreset = {
    /** Square-ish category / service card image. */
    category: (url) => (0, exports.cldUrl)(url, { width: 400, height: 300, crop: 'fill', gravity: 'auto' }),
    /** Round profile / avatar image. Uses g_auto (safe for non-face profile pictures). */
    avatar: (url, size = 200) => (0, exports.cldUrl)(url, { width: size, height: size, crop: 'fill', gravity: 'auto' }),
    /** Small list thumbnail. */
    thumb: (url, size = 96) => (0, exports.cldUrl)(url, { width: size, height: size, crop: 'fill' }),
    /** Wide promotional banner. */
    banner: (url, width = 1000) => (0, exports.cldUrl)(url, { width, crop: 'limit' }),
};
exports.default = exports.cldUrl;
