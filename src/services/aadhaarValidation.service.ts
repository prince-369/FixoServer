...
const digitNormalise = (raw: string): string => raw.replace(/[OoDQIlLi|!ZzSsGB]/g, (ch) => OCR_TO_DIGIT[ch] ?? ch);
...
const findValidAadhaarDigits = (raw: string): string | undefined => {
  for (const variant of [raw, digitNormalise(raw)]) {
    const candidates = variant.match(/[2-9]\d{3}\s*\d{4}\s*\d{4}/g) || [];
    for (const c of candidates) {
      const digits = c.replace(/\D/g, '');
      if (digits.length === 12 && isValidAadhaarNumber(digits)) return digits;
    }
  }
  return undefined;
};

const hasVerhoeffValidNumber = (raw: string): boolean => !!findValidAadhaarDigits(raw);
