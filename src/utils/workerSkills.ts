...
export const syncCategoriesFromSkills = (worker: { skills?: SkillLike[]; categories?: unknown }) => {
  const approved = (worker.skills || [])
    .filter((s) => s.status === 'approved')
    .map((s) => s.category);
  (worker as { categories: unknown }).categories = approved;
};

// Account age in whole months (~30 day months).
export const accountAgeMonths = (createdAt?: Date | string | null): number => {
...
export const maxExperienceBumps = (createdAt?: Date | string | null): number =>
  Math.floor(accountAgeMonths(createdAt) / 6);

// Parse the skills payload sent at registration / skill-request.
export interface SkillInput { categoryId: string; experienceYears: number; confirmed: boolean }
export const parseSkillsInput = (raw: unknown): SkillInput[] => {
  let arr: unknown = raw;
